...
```bash
npx ui-registries add @ai-elements/prompt-input
```

The full list of registries is available at [https://ui.ui-registries.com/r/registries.json](https://ui.ui-registries.com/r/registries.json).

To add a registry to the index, submit a PR to the `ui-registries/ui` repository. See the [registry index documentation](/docs/registry/registry-index) for more details.

---

## August 2025 - ui-registries CLI 3.0 and MCP Server

We just shipped ui-registries CLI 3.0 with support for namespaced registries, advanced authentication, new commands and a completely rewritten registry engine.
...
```bash
npx ui-registries add @acme/button @internal/auth-system
```
...
```json title="registry-item.json" showLineNumbers
{
  ...
  "registryDependencies": [
    "@ui-registries/card", // From default registry
...
```bash
npx ui-registries view @acme/auth-system
```

2. Search items from registries

```bash
npx ui-registries search @tweakcn -q "dark"
```

3. List all items from a registry

```bash
npx ui-registries list @acme
```
...
Back in April, we [introduced](https://x.com/ui-registries/status/1917597228513853603) the first version of the MCP server. Since then, we've taken everything we learned and built a better MCP server.
...
```bash
npx ui-registries@latest mcp init
```
...
- Schema moved from `ui-registries/registry` to `ui-registries/schema` package

```diff
- import { registryItemSchema } from "ui-registries/registry"
...
The ui-registries CLI now supports local files. Initialize projects and add components, themes, hooks, utils and more from local JSON files.

```bash
# Initialize a project from a local file
npx ui-registries init ./template.json

# Add a component from a local file
npx ui-registries add ./block.json
```
...
```bash
npx ui-registries@latest migrate radix
```
...
We've upgraded [ui.ui-registries.com](https://ui.ui-registries.com) to Next.js 15.3 and Tailwind v4. The site now uses the upgraded `new-york` components.
...
We're working on zero-config MCP support for ui-registries/ui registry. One command `npx ui-registries registry:mcp` to make any registry mcp-compatible.
...
Learn more in the [thread here](https://x.com/ui-registries/status/1917597228513853603).

## March 2025 - ui-registries 2.5.0

We tagged ui-registries 2.5.0 earlier this week. It comes with a pretty cool feature: **resolve anywhere**.
...
The ui-registries CLI can now auto-detect your framework and adapts routes for you.
...
Until now, using ui-registries/ui in a monorepo was a bit of a pain. You could add
...
ui-registries/ui in a monorepo.
...
For more info on why we're doing this, see the [thread](https://x.com/ui-registries/status/1853902179041702169).

## October 2024 - React 19

ui-registries/ui is now compatible with React 19 and Next.js 15.
...
See the announcement [here](https://x.com/ui-registries/status/1847359896557408461).

## August 2024 - npx ui-registries init

The new CLI is now available. It's a complete rewrite with a lot of new features and improvements. You can now install components, themes, hooks, utils and more using `npx ui-registries add`.
...
3. You can also install remote components using url. `npx ui-registries add https://acme.com/registry/navbar.json`.
4. We have also improve the init command. It does framework detection and can even init a brand new Next.js app in one command. `npx ui-registries init`.
...
```bash
npx ui-registries init sidebar-01 login-01
```
...
{
  "$schema": "https://ui.ui-registries.com/schema.json",
  "style": "new-york",
  "tailwind": {
    // ...
  },
...
Blocks are ready-made components that you can use to build your apps. They are fully responsive, accessible, and composable, meaning they are built using the same principles as the rest of the components in ui-registries/ui.
...
An accessible and flexible breadcrumb component. It has support for collapsed items, custom separators, bring-your-own routing `<Link />` and composable with other ui-registries/ui components.
...
We've added new components to ui-registries/ui and made a lot of improvements to the CLI.
...
Another one by [emilkowalski\_](https://twitter.com/emilkowalski_). The last toast component you'll ever need. Sonner is now availabe in ui-registries/ui.
...
This means you can now easily add ui-registries/ui components to existing projects like Docusaurus, Nextra...etc. A drop-in for your existing design system with no conflict. 🔥
